...
const unRootPath = str => str.replace('root.', '').replace('root', '')
...
const prepListenerObjects = function (state, listeners, log, noValueDefault) {
    ...
    for (let listenerConfig of listeners) {
        for (let listenerPath of Array.isArray(listenerConfig.path)
            ? listenerConfig.path
            : [listenerConfig.path]) {
            if (
                listenerPath !== '' &&
                get(state, listenerPath, undefined) === undefined
            ) {
                log.error(
                    `Incompatible state and listener on path "${listenerPath}"`
                )
                // throw new Error( `Incompatible state and listener on path "${listenerPath}"` )
            }

            const id = listenerConfig.id
            const pathToListenerSet =
                'root' +
                (listenerPath ? '.' : '') +
                listenerPath +
                '["%listeners%"]'
            const handlerSet = {
                [id]: listenerConfig,
                ...get(listenerObject, pathToListenerSet, {}),
            }
...
const createPathListenerDictionary = (
    obj,
    pathPrefix,
    ...
    for (let [key, value] of Object.entries(obj)) {
        ...
        const listenersDefinedForPath = get(
            listenerRegisterObject,
            pathToHandle + '["%listeners%"]',
            false
        )
        ...
        if (listenersDefinedForPath) {
            ...
            nextParentListenerSet = {
                listeners: Object.values(listenersDefinedForPath),
                path: pathToHandle,
            }
        }
        ...
        if (typeof value === 'object' && value !== null) {
            ...
            for (let subListeners of Object.values(subPathListenerDictionary)) {
                childListenerSets.push(
                    ...subListeners.childListenerSets.filter(
                        lSet => !childListenerSets.includes(lSet)
                    )
                )

                if (
                    subListeners.parentListenerSet !== nextParentListenerSet &&
                    !childListenerSets.includes(subListeners.parentListenerSet)
                ) {
...
const wrapListenerArrays = function (
    pathListenerDictionary,
    indexedState,
    log
) {
    const wrappedListenerRegister = {}

    for (let [statePath, relevantListenerSets] of Object.entries(
        pathListenerDictionary
    )) {
        ...
        if (!relevantListenerSets.parentListenerSet.listeners) {
            ...
            wrappedListenerRegister[rootLessChangePath] =
                createUnhandledPathHandler(statePath, log)
            continue
        }
        ...
        const parentListenerHandlers = uniq(
            relevantListenerSets.parentListenerSet.listeners.map(l => l.handler)
        )

        // all of the paths that the parent listeners listen to (as a listener can listen/sbuscribe to more than one path)
        const parentListenerPaths = uniq(
            flatten(
                relevantListenerSets.parentListenerSet.listeners.map(
                    l => l.path
                )
            )
        )
        ...
        const hasChildrenListeners =
            relevantListenerSets.childListenerSets.length > 0
        ...
        const childrenListenerHandlers = uniq(
            flattenDeep(
                relevantListenerSets.childListenerSets.map(cls =>
                    cls.listeners.map(l => l.handler)
                )
            )
        )
        const childrenListenerPaths = uniq(
            flattenDeep(
                relevantListenerSets.childListenerSets.map(cls =>
                    cls.listeners.map(l => l.path)
                )
            )
        )

        // bundlify
        const bundledChildrenListenerHandlers = bundleFns(
            childrenListenerHandlers
        )

        // paths without { root: <state> } wrapping
        const rootLessParentListPath = unRootPath(
            relevantListenerSets.parentListenerSet.path
        )
        const childrensPaths = relevantListenerSets.childListenerSets.map(cls =>
            unRootPath(cls.path)
        )

        // this function updates the cached state values for all children listeners
        const updateChildrensCaches = bundleFns(
            childrensPaths.map(
                cp => state =>
                    (indexedState[cp].cache = indexedState[cp].lens(state.data))
            )
        )
        ...
        const parentCacheUpdateFn =
            rootLessParentListPath === rootLessChangePath
                ? function (changeValue) {
                      indexedParentListState.cache = changeValue
                  }
                : function (changeValue, state) {
                      indexedParentListState.cache =
                          indexedParentListState.lens(state.data)
                  }

        // define the listener
        wrappedListenerRegister[rootLessChangePath] = function (
            changeValue,
            state,
            verbose = false
        ) {
            ...
            if (changeValueType === 'object') {
                ...
                // sanity checking
                throw new Error(
                    `Unacceptable state change: this listener has children, can't push update with primitive`
                )
            }
            ...
            bundledParentListenerHandlers(lensedSubStates, {
                path: rootLessChangePath,
                value: changeValue,
            })
            ...
            if (changeValueType === 'object') {
                ...
                bundledChildrenListenerHandlers(lensedChildrenSubStates, {
                    path: rootLessChangePath,
                    value: changeValue,
                })
            }
...
const remapListeners = function (state, listeners, log, noValueDefault) {
    log.debug(`StoreBot remap listeners`)
    const t = Date.now()
    const { listenableSpace, listenerObject, indexedState } =
        prepListenerObjects(state.data, listeners, log, noValueDefault)

    const pathToListenerArrayDictionary = createPathListenerDictionary(
        listenableSpace,
        '',
        listenerObject,
        []
    ...
    const pathToWrappedListenerDictionary = wrapListenerArrays(
        pathToListenerArrayDictionary,
        indexedState,
        log
    )

    log.debug(`StoreBot listener remapping took ${Date.now() - t}ms`)

    return pathToWrappedListenerDictionary
}

/**
 * @typedef {Object} StoreAPIGetterSpec
 * @prop {string} name
 * @prop {string[]} storePath
 */

/**
 * @typedef {Object} StoreAPISetterSpec
 * @prop {string} name
 * @prop {string} storePath
 */
...
const createStore = function ({
    ...
    if (!initialState) {
        throw new Error('StoreBot needs an initialState')
    }
    ...
    let mappedListeners = remapListeners(
        cachedState,
        listenerConfigs,
        log,
        noValueDefault
    ...
    const remapCurrentListeners = () => {
        mappedListeners = remapListeners(
            cachedState,
            listenerConfigs,
            log,
            noValueDefault
    ...
    const addListener = function (listenerConfig, { remap = true } = {}) {
        if (Array.isArray(listenerConfig)) {
            return listenerConfig.map(singleListenerConfig =>
                addListener(singleListenerConfig)
            )
        }
        ...
        if (remap !== false) {
            try {
                ...
                log.error(err)
                throw new Error(
                    `Could not add listener for "${listenerConfig.path}". ${err.message}`
                )
            }
        }
    ...
    const removeListener = function (id) {
        ...
        try {
            ...
            log.error(err)
            throw new Error(
                `Could not remove listener for "${listenerConfig.path}". ${err.message}`
            )
        }
    }
    ...
    const updateState = function (state, forceRemap) {
        ...
        if (!handler || forceRemap) {
            if (!handler) {
                log.info(
                    `Unknown state path "${lsPath}" -> remapping listeners`
                )
            }
            if (forceRemap) {
    ...
    const readCachedValue = (storePath, def = null) => {

        return JSON.parse(JSON.stringify(get(cachedState.data, storePath, def)))
    }

    /** @function createAPI
     * @param {StoreAPIGetterSpec[]} getters
     * @param {StoreAPISetterSpec[]} setters
     * @param {StoreListener[]} listeners
     */
    ...
    const createAPI = function ({
        ...
        for (let getter of getters) {
            Object.defineProperty(api, getter.name, {
                get: () => readCachedValue(getter.storePath, null),
            })
        }
        for (let setter of setters) {
            api[ setter.name ] = async val => updateValue(setter.storePath, val);
        }

        let listenerIds = []
        for (let listener of listeners) {
            listenerIds.push(addListener(listener))
        }
